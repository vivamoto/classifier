...
def ME(X, Yd, m = 4, gating = 'linear', add_bias = False):
    ...
    it = 0
    maxiter = 100
    while abs(lik_new - lik_old) > 1e-2 and it < maxiter:
        ...
        # Gaussian gating
            ...
            try:
                ...
                print("Problemas no h.")
            # Step M: optimize gating and expert outputs
            for i in range(m):
                wexp[:,:,i], var[i] = atualiza_exp(X, Yd, h[:, i], var[i], wexp[:,:,i], ns)
            alpha = np.sum(h, axis=0) / np.sum(h + eps)
            alpha = np.sum(h, axis=0, keepdims=True).T / h.shape[0]
            ...
            if it > 2 and gating == 'gaussian':
                if lik_new < lik_old:
                    print("Não converge")
                elif lik_new < lik_old + 1e-3:
                    print("Convergiu")
                break
...
def calc_prob_exp(yexp, Yd, var):
    ...
    for i in range(m):
        Y = yexp[:,i:i+1]
        for n in range(N):
            p[n, i] = (1 / ((2 * np.pi * var[i]) ** (ns / 2))) * \
                      np.exp(-np.linalg.norm(Yd[n,:] - Y[n,:])**2/(2*var[i]))  # Victor (slide 129)

    p = np.exp(-(Yd - yexp)**2 /(2*var)) / np.sqrt(2*np.pi*var)
...
def prob(X, alpha, gamma, sigma):
    ...
    for i in range(m):
        ...
        for t in range(N):
            try:
                dist[t,:] = dif[t,:] @ inv @ dif[t,:].T
            except:
                print("Problema na função prob")
        p_xv[:, i:i+1] = np.exp(-dist/2) / (eps + ((2 * np.pi) ** (ne / 2)) * np.sqrt(np.linalg.det(sigma[:, :, i])))
...
if __name__ == '__main__':
    ...
    # 1. Load the dataset
    # ------------------------------------------------
    ts = np.loadtxt("data/mistura_especialista_conj_treinamento.txt")

    # ------------------------------------------------
    # 2. Create charts to select the best time series lag
    # ------------------------------------------------
    best_lag(ts, pdir = pdir)
    lag = 20

    # ------------------------------------------------
    # 3. Cross validation for choosing the number of experts
    # ------------------------------------------------
    cv_experts(ts = ts,  lag = lag, gating = 'linear', pdir = 'Report_ME/')
    cv_experts(ts = ts,  lag = lag, gating = 'gaussian', pdir = 'Report_ME/')

    # ------------------------------------------------
    # 4. Train and test ME
    # ------------------------------------------------
    final_ME(ts = ts, m = 6, lag = lag, gating = 'linear',   pdir = 'Report_ME/')
    final_ME(ts = ts, m = 2, lag = lag, gating = 'gaussian', pdir = 'Report_ME/')
